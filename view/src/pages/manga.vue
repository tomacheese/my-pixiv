<template>
  <v-container fluid>
    <ItemList
      :items="items"
      :loading="loading"
      :vieweds="vieweds"
      @open="open"
      @intersect-item="onItemViewing"
    ></ItemList>
    <v-dialog v-model="overlay.isMangaOpened">
      <IllustPopup :item="overlay.target" @close-popup="close()"></IllustPopup>
    </v-dialog>
  </v-container>
</template>

<script lang="ts">
import Vue from 'vue'
import { PixivItem, PixivItemWithSearchTag } from '@/types/pixivItem'
import { Fetcher } from '@/plugins/fetcher'
import { TargetType } from '@/store/settings'

export default Vue.extend({
  name: 'MangaPage',
  data(): {
    items: PixivItemWithSearchTag[]
    vieweds: number[]
    page: number
    overlay: {
      isMangaOpened: boolean
      target: PixivItem | null
    }
    loading: boolean
  } {
    return {
      items: [],
      vieweds: [],
      page: 1,
      overlay: {
        isMangaOpened: false,
        target: null,
      },
      loading: false,
    }
  },
  async created() {
    await this.fetch()

    this.vieweds = this.$accessor.viewed.illusts
  },
  methods: {
    async fetch() {
      this.loading = true

      const targetType: TargetType = 'MANGA'
      const fetcher = new Fetcher(
        this.$config,
        this.$axios,
        this.$accessor.settings.filters,
        targetType
      )
      this.items = await fetcher.getItems(
        this.$accessor.settings.specificTargets(targetType)
      )

      this.loading = false
    },
    open(item: PixivItem): void {
      this.overlay.isMangaOpened = true
      this.overlay.target = item
    },
    close() {
      this.overlay.isMangaOpened = false
      this.overlay.target = null
    },
    onItemViewing(item: PixivItem) {
      this.$accessor.viewed.addIllust(item)
    },
  },
})
</script>
